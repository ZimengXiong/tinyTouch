...
bool firmware_update_write(size_t offset, const uint8_t *data, size_t length) {
  if (!active || !data || offset != written_size || length == 0 ||
...
bool firmware_update_commit(void) {
  if (!active || written_size != expected_size) return false;
...
bool firmware_update_active(void) { return active; }
bool firmware_update_staged(void) { return staged; }
